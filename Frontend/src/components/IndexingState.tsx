  ...
  return (
    <div className="flex flex-col">
      <div className="flex items-center gap-3">
        ...
        <span
          className={`font-body-sm transition-colors text-sm ${
            isCompleted
...
export const IndexingState: React.FC<IndexingStateProps> = ({
  ...
  return (
    <div
      id="indexing-screen"
      className="w-full min-h-[calc(100vh-60px)] lg:h-[calc(100vh-60px)] bg-surface-50 overflow-y-auto lg:overflow-hidden"
    >
      {/* ===================== MOBILE VIEW (< lg) ===================== */}
      {/* Requested Order: 1. My Resume -> 2. Container Loading -> 3. Analyse Loading */}
      <div className="flex lg:hidden flex-col w-full p-4 sm:p-6 space-y-6">
        {/* 1. My Resume Section */}
        <div className="flex flex-col">
          <span className="font-label-micro text-ink-500 tracking-[0.2em] uppercase mb-4 font-semibold text-xs">
            YOUR RESUME
          </span>
          ...
          <div className="bg-white rounded-xl p-5 border border-border-200 shadow-[0_1px_3px_rgba(15,23,42,0.04)] flex items-center gap-3.5 mb-3">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="font-label-regular text-ink-500 mt-0.5 font-medium text-xs">
                {resume.sizeKb} KB
              </span>
            </div>
          </div>
          ...
          <button
            onClick={onCancel}
            className="w-full flex items-center justify-center gap-2 py-2.5 px-3 border border-border-200 hover:border-ink-300 text-ink-900 hover:bg-surface-100 rounded-lg font-label-regular transition-colors bg-white cursor-pointer text-sm"
            type="button"
          >
        ...
        {/* 2. Container Loading Section */}
        <div className="w-full flex justify-center py-4 px-2 sm:px-4 bg-white bg-dot-grid rounded-2xl border border-border-200 relative overflow-hidden">
          {/* Subtle ambient light behind card */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-brand-100/25 rounded-full blur-2xl pointer-events-none" />

          <TiltCard className="flex flex-col items-center text-center w-full max-w-lg z-10 p-6 sm:p-8 rounded-2xl bg-white border border-border-200 shadow-[0_16px_40px_rgba(15,23,42,0.08),0_0_40px_rgba(204,251,241,0.3)] relative overflow-hidden group">
            ...
            <h2 className="font-headline-lg text-xl sm:text-headline-lg font-bold text-ink-900 mb-2 tracking-tight">
              Building your resume index…
            </h2>
            <p className="font-body-md text-sm text-ink-500 mb-6 max-w-sm leading-relaxed">
            ...
            <div className="w-full flex items-center justify-between text-xs text-ink-500 font-label-micro px-0.5">
              <span>{subStatus}</span>
        ...
        {/* 3. Analyse Loading Section */}
        <div className="flex flex-col pt-2 pb-6">
          <span className="font-label-micro text-ink-500 tracking-[0.2em] uppercase mb-5 font-semibold text-xs">
            ANALYSING
          </span>
          ...
          <div className="pt-6 mt-6 border-t border-border-200 flex items-center justify-between text-ink-500">
            <span className="font-label-micro uppercase tracking-widest text-ink-500 font-semibold text-xs">
              ChromaDB Vector Store
            </span>
            <span className="font-label-micro font-semibold text-success-600 flex items-center gap-1.5 text-xs">
      ...
      <div className="hidden lg:flex flex-row w-full min-h-[calc(100vh-60px)]">
        {/* Left Column (Ingestion & Pipeline Dossier Stage) */}
        <aside
          id="indexing-sidebar"
          className="w-85 shrink-0 bg-surface-50 border-r border-border-200 p-6 flex flex-col justify-between"
        >
          <div className="flex flex-col">
            ...
            <button
              onClick={onCancel}
              className="w-full flex items-center justify-center gap-2 mb-6 py-2 px-3 border border-border-200 hover:border-ink-300 text-ink-900 hover:bg-surface-100 rounded-lg font-label-regular transition-colors bg-white cursor-pointer"
              type="button"
            >
              <FileUp size={18} className="text-brand-600" />
              <span className="font-medium">Upload another resume</span>
            ...
            {/* Pipeline Progress Section (ANALYSING) */}
            <div className="flex flex-col">
              <span className="font-label-micro text-ink-500 tracking-[0.2em] uppercase mb-4 font-semibold">
                ANALYSING
              </span>
        ...
        <section
          id="indexing-stage"
          className="flex-1 bg-white bg-dot-grid flex flex-col items-center justify-center p-12 relative overflow-hidden"
        >
          ...
          <TiltCard className="flex flex-col items-center text-center w-full max-w-lg z-10 p-8 rounded-2xl bg-white border border-border-200 shadow-[0_16px_40px_rgba(15,23,42,0.08),0_0_40px_rgba(204,251,241,0.3)] relative overflow-hidden group">
            <div className="absolute top-0 left-0 right-0 h-0.5 bg-linear-to-r from-brand-500 to-brand-600"></div>
            <div className="absolute -top-16 -right-16 w-36 h-36 rounded-full bg-intel-50 blur-2xl pointer-events-none"></div>
